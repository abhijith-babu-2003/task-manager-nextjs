...
export async function verifyToken(token) {
  try {
    ...
    // In browser environment, return decoded token without DB check
    ...
    // Server-side: Ensure DB is connected
    try {
      await initDB();
    } catch (dbError) {
      console.error('verifyToken: Database connection failed:', dbError);
      // Return decoded token even if DB check fails to prevent complete auth failure
      return {
        userId: userId,
        id: userId,
        email: decoded.email,
    ...
    try {
      ...
      // Return decoded token data if user lookup fails
      return {
        userId: userId,
        id: userId,
        email: decoded.email,
...
export function setAuthToken(response, token) {
  ...
  try {
    // For Next.js response with cookies API
    ...
    // For client-side (should rarely be used directly)
